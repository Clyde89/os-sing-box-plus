...
for command_name in pkg sha256 stat service configctl ifconfig route ps awk sed grep; do
...
for owned_path in "$SING_BOX_BINARY" "$RC_FILE" "$POLICY_READINESS" "$SYSTEM_RESOLVER_HELPER" "$BUILD_INFO"; do
...
check_mode "$SYSTEM_RESOLVER_HELPER" 755 "helper системного resolver"
...
check_mode "$SYSTEM_RESOLV_CONF" 644 "системный resolv.conf"
SYSTEM_NAMESERVERS="$(
    awk '$1 == "nameserver" {print $2}' "$SYSTEM_RESOLV_CONF" 2>/dev/null || true
)"
if grep -Fq '# This file was automatically generated by system_resolvconf_generate()' "$SYSTEM_RESOLV_CONF" 2>/dev/null \
    && [ -n "$SYSTEM_NAMESERVERS" ]; then
    pass "системный resolv.conf был сгенерирован OPNsense"
else
    fail "системный resolv.conf не принадлежал штатному генератору OPNsense"
fi

RESOLVER_USES_TUN=0
for nameserver in $SYSTEM_NAMESERVERS; do
    RESOLVER_INTERFACE="$(
        route -n get "$nameserver" 2>/dev/null |
            awk '/interface:/ {print $2; exit}'
    )"
    if [ "$RESOLVER_INTERFACE" = "$TUN_INTERFACE" ]; then
        RESOLVER_USES_TUN=1
        break
    fi
done
if [ "$RESOLVER_USES_TUN" -eq 0 ]; then
    pass "системный resolver не зависел от TUN sing-box"
else
