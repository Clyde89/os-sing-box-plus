...
grep -q '/api/singbox/settings/adopt' "$VIEW"
grep -q 'Сохранить настройки' "$VIEW"
grep -q 'Предварительный просмотр' "$VIEW"
grep -q 'Применить runtime-конфигурацию' "$VIEW"
grep -q 'Подтвердить безопасный переход' "$VIEW"
grep -q 'adoptionConfirm' "$VIEW"
grep -q 'adoption_ready' "$VIEW"
...
grep -q 'policyDnsBootstrap' "$VIEW"
grep -q 'networkPreflightState' "$VIEW"
grep -q 'networkPreflightErrors' "$VIEW"
grep -q 'Сетевой preflight' "$VIEW"
...
grep -q '<id>settings.dns.policyUpstreamAddress</id>' "$FORM"
grep -q '<id>settings.dns.policyUpstreamTlsServerName</id>' "$FORM"
...
grep -q '<version>1.5.0</version>' "$MODEL"
grep -q '<ConfigdPopulateAct>interface gateways list -l</ConfigdPopulateAct>' "$MODEL"
grep -q '<policyUpstreamType type="OptionField">' "$MODEL"
grep -q '<policyUpstreamAddress type="NetworkField">' "$MODEL"
grep -q '<policyUpstreamPort type="PortField">' "$MODEL"
...
grep -q "'dns_bootstrap'" "$API_CONTROLLER"
grep -q 'function adoptAction' "$API_CONTROLLER"
grep -q 'function runtimeNetworkPreflight' "$API_CONTROLLER"
grep -q "configdRun('sing-box preflight')" "$API_CONTROLLER"
grep -q "'network_preflight'" "$API_CONTROLLER"
